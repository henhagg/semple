...
for method in methods_list:
    for run in observation_list:
        filename = path_results / f'{method}/run{run}/c2st.csv'
        mtr.append(pd.read_csv(filename))
        filename = path_results / f'{method}/run{run}/elapsed_time.csv'
        eti_mo = pd.read_csv(filename, header=None, names=['time'])
        eti_mo['num_run'] = run
        eti_mo['algorithm'] = method
        if method == 'semple/10k':
...
mtr_df.loc[mtr_df['algorithm'] == 'JASS', 'algorithm'] = 'semple/10k'
mtr_df.loc[mtr_df['algorithm'] == 'SNPE', 'algorithm'] = 'snpe/40k_10rounds'
mtr_df.loc[mtr_df['algorithm'] == 'SNLE', 'algorithm'] = 'snle/10rounds_resample'
...
plt.subplots_adjust(top=0.95, left=0.10,  right=0.95)
colors = ['C0', 'C1', 'C2']
