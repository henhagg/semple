...
def plot_kde_single_param_all_iter(input_dir, parameter_index, true_value=None):
    with open(f"{input_dir}/settings.json", "r") as openfile:
        settings_dict = json.load(openfile)
    num_iters = settings_dict["num_rounds"]
...
def plot_kde_multi_param_all_iter(input_dir, true_values, param_name_list, xlim_list=None, save_path=None):
    with open(f"{input_dir}/settings.json", "r") as openfile:
        settings_dict = json.load(openfile)
    num_iters = settings_dict["num_iters"]

    num_param = len(true_values)
    num_grid_rows = int(np.ceil(num_param/2))

    fig, axes = plt.subplots(num_grid_rows, 2, figsize=(7,7))
    axes_index_list = [axes[0,0], axes[1,0], axes[2,0], axes[0,1], axes[1,1], axes[2,1]]
    # axes_index_list = [axes[0,0], axes[0,1], axes[1,0]]
    legend_list = [f'iteration {i}' for i in range(0,num_iters+1)]
    ...
    for param_index in range(num_param):
        ...
        if(xlim_list):
            axes_index_list[param_index].set_xlim(xlim_list[param_index])
...
def plot_kde_multi_param_all_iter_2(input_dir, num_rounds, true_values, param_name_list, subplot_rows, subplot_cols, figsize=(7,7), xlim_list=None, save_fig=False):
    num_param = len(true_values)

    fig, axes = plt.subplots(subplot_rows, subplot_cols, figsize=figsize)
    axes_index_list = axes.T.reshape(-1)
    legend_list = [f'round {i}' for i in range(0,num_rounds+1)]

    for param_index in range(num_param):
        for round in range(0, num_rounds+1):
            file_path = f"{input_dir}/post_sample_iter{round}.csv"
            posterior_samples = pd.read_csv(file_path, index_col=False, header=None)
            posterior_samples_param = posterior_samples.iloc[:,param_index]
            sns.kdeplot(ax=axes_index_list[param_index], data=posterior_samples_param)
    ...
    plt.tight_layout()
    if(save_fig):
        # save_path = f"{input_dir}/kde.pdf"
        # plt.savefig(save_path)
        save_path = f"{input_dir}/kde.png"
        plt.savefig(save_path)
    else:
        plt.show()
